...
public class BookingController {
    ...
    public ApiResponse<List<BookingResponse>> getBookingByUserId(@PathVariable("userId") UUID userId){
        return ApiResponse.<List<BookingResponse>>builder().result(bookingService.getMyBookings(userId)).build();
    }

    @GetMapping("/{bookingId}")
    public ApiResponse<BookingResponse> getBookingById(@PathVariable("bookingId") UUID bookingId){
        System.out.println(bookingId);
        return ApiResponse.<BookingResponse>builder().result(bookingService.getBookingById(bookingId)).build();
    }

    @GetMapping
    public ApiResponse<List<BookingResponse>> getBookingByUserId(){
        return ApiResponse.<List<BookingResponse>>builder().result(bookingService.getAll()).build();
    }
    ...
    public ApiResponse<BookingResponse> updateBooing(@PathVariable("bookingId") UUID bookingId, @RequestBody BookingUpdateRequest request){
        return ApiResponse.<BookingResponse>builder().result(bookingService.updateBooking(bookingId, request)).build();
    }

    @DeleteMapping("/{bookingId}")
    public ApiResponse<String> deleteBooking(@PathVariable("bookingId") UUID bookingId){
        return ApiResponse.<String>builder().result(bookingService.deleteBooking(bookingId)).build();
    }
}
