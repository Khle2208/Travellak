...
public interface BookingRepository extends JpaRepository<Booking, UUID> {
    ...
    @Query(
			...
			""")
    int cancelExpiredBookings(@Param("today") LocalDate today, @Param("pendingStatus") String pendingStatus);
    ...
    @Query(
            "SELECT COALESCE(SUM(b.totalPrice), 0) FROM Booking b WHERE b.status = :status AND MONTH(bookingDate) = :month")
