...
public class AttractionsController {
    ...
    public ApiResponse<AttractionsResponse> createAttraction(@ModelAttribute AttractionsCreationRequest request) {
        return ApiResponse.<AttractionsResponse>builder()
                .result(attractionsService.createAttraction(request))
                .build();
    }

    @GetMapping
    public ApiResponse<List<AttractionsResponse>> getAllAttractions() {
        return ApiResponse.<List<AttractionsResponse>>builder()
                .result(attractionsService.getAllAttractions())
                .build();
    }
    ...
    public ApiResponse<AttractionsResponse> getAttractionById(@PathVariable("attractionId") UUID attractionId) {
        return ApiResponse.<AttractionsResponse>builder()
                .result(attractionsService.getAttractionById(attractionId))
                .build();
    }

    @PutMapping(value = "/{attractionId}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponse<AttractionsResponse> updateAttraction(
            @PathVariable("attractionId") UUID attractionId, @ModelAttribute AttractionsUpdateRequest request) {
        return ApiResponse.<AttractionsResponse>builder()
                .result(attractionsService.updateAttraction(attractionId, request))
                .build();
    }
